...
#endif
  std::cout << "DB NUM MOVABLE NODES IS " << db.num_movable_nodes << std::endl;
...
#endif
  std::cout << "STATE BATCH SIZE IS " << state.batch_size << std::endl;
...
int globalSwapCUDALauncher(DetailedPlaceDB<T> db, int batch_size, int max_iters,
                           int num_threads) {
  std::cout << "BATCH SIZE IN GLOBAL SWAP CUDA LAUNCHER IS " << batch_size << std::endl;
  dreamplacePrint(
